...
class GamePanel {
    ...
    drawOtherPlayer(player) {
        ...
        // Calculate camera offset correctly - based on player's worldX/worldY if available
        const playerWorldX = this.player.worldX || this.player.x;
        const playerWorldY = this.player.worldY || this.player.y;
        ...
        const cameraX = screenCenterX - playerWorldX;
        const cameraY = screenCenterY - playerWorldY;
        
        // Choose sprite based on direction and player class
        let sprite;
        const className = player.characterClass || 'mage';
        
        // If you have the sprites loaded for each class, use them
        if (player.direction === 'up') {
            sprite = this.player.up1; // Use the main player's sprite as fallback
        ...
        // Draw the sprite if available, otherwise fall back to colored rectangle
        ...
        ctx.strokeText(
            displayName,
            player.x + cameraX + this.tileSize/2,
            player.y + cameraY - 10
        );
...
class MultiplayerManager {
    ...
    connect() {
        try {
            ...
            // Ensure game continues even if multiplayer fails
            this.game.ui.showMessage("Multiplayer unavailable", 3000);
        }
    }
    ...
    sendPosition() {
        ...
        this.socket.send(JSON.stringify({
            ...
            characterClass: this.game.player.characterClass, // Add character class
            name: this.game.player.name // Add player name
        }));
    }
